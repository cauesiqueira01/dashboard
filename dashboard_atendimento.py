        ...
        html.Div([
            dash_table.DataTable(
                ...
                columns=[
                    ...
                    {'name': 'Data Abertura', 'id': 'Data/hora abertura'},
                    {'name': 'Data Encerramento', 'id': 'Data/hora encerramento'},
                    {'name': 'TIT', 'id': 'TIT'},
                    {'name': 'SLA', 'id': 'SLA 2'},
...
def atualizar_tendencia_chamados(empresa_selecionada):
    ...
    df_entrantes['Mes'] = df_entrantes['Data/hora abertura'].dt.to_period('M')
    df_fechados['Mes'] = df_fechados['Data/hora encerramento'].dt.to_period('M')
    
    entrantes_por_mes = df_entrantes.groupby('Mes').size()
    fechados_por_mes = df_fechados.groupby('Mes').size()
...
def atualizar_tabela_detalhes(empresa, tit, sla):
    ...
    df_combinado['Data/hora abertura'] = df_combinado['Data/hora abertura'].dt.strftime('%d/%m/%Y %H:%M')
    df_combinado['Data/hora encerramento'] = df_combinado['Data/hora encerramento'].dt.strftime('%d/%m/%Y %H:%M')
